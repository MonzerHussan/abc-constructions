...
export default function ZoneCard({ zone, className }: { zone: ZoneView; className?: string }) {
  ...
  const hasVideo = isUsableMediaUrl(zone.videoUrl)
  const imageUrl = isUsableMediaUrl(zone.imageUrl) ? zone.imageUrl.trim() : null
  const posterUrl = isUsableMediaUrl(zone.posterUrl)
    ? zone.posterUrl!.trim()
    : imageUrl ?? undefined
  ...
  const media = isVideo && hasVideo ? (
    ...
    <Image src={imageUrl} alt={zone.title} fill className="object-cover" />
  ) : null
