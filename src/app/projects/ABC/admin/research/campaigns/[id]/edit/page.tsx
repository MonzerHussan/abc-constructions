...
export default function EditCampaignPage() {
  ...
  async function fetchCampaign() {
    try {
      ...
      setError(t("researchCampaignLoadFailed"))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json()
        throw new Error(d.error || t("researchCampaignUpdateFailed"))
      }
  ...
  return (
    <AdminSurveyShell
      title={t("researchCampaignEditTitle")}
      loading={loading}
      actions={
    ...
    >
      <form onSubmit={handleSubmit} className="bg-white border rounded-xl p-6 space-y-5 max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">{t("researchCampaignFormStatus")}</label>
            <select
              name="status"
              value={form.status}
              onChange={handleChange}
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={submitting} className={ADMIN_ACTION_BTN}>
            <Save size={16} /> {submitting ? t("researchSaving") : t("researchCampaignSaveChanges")}
          </button>
          <Link href={`/projects/ABC/admin/research/campaigns/${params.id}`} className={ADMIN_ACTION_BTN_SECONDARY}>
            {t("researchCancel")}
          </Link>
        </div>
      </form>
