      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-surface-600 mt-1">{t("jobsDescription")}</p>
          </div>
          <Link
            href="/projects/ABC/jobs/new"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="p-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none"
                />
              </div>
            </Card>
                ...
                <Card key={job.id} hover className="p-5">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h3 className="text-base font-bold text-surface-900 hover:text-teal-600 cursor-pointer transition-colors">
                              {job.title}
                            </h3>
                            {job.isUrgent && (
                              <span className="flex items-center gap-0.5 text-xs font-bold text-danger-600 bg-danger-50 px-1.5 py-0.5 rounded-full">
                                <Zap className="w-3 h-3" />
                                {language === "ar" ? "عاجل" : language === "en" ? "Urgent" : "فوری"}
                              </span>
                            )}
                            {job.easyApply && (
                              <span className="flex items-center gap-0.5 text-xs font-medium text-info-600 bg-info-50 px-1.5 py-0.5 rounded-full">
                                <Sparkles className="w-3 h-3" />
                                {language === "ar" ? "تقديم سريع" : language === "en" ? "Easy Apply" : "آسان درخواست"}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-surface-600 mt-0.5">{job.company.name}</p>
                        </div>
                        <button
                          onClick={() => toggleSave(job.id)}
                          className="p-1.5 rounded-lg hover:bg-surface-100 transition-colors flex-shrink-0"
                        >
                          <Bookmark className={`w-5 h-5 ${savedJobs.has(job.id) ? "text-teal-600 fill-current" : "text-surface-400"}`} />
                        </button>
                      </div>

                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-sm text-surface-500">
                      ...
                      <p className="text-sm text-surface-600 mt-2 line-clamp-2">{job.description}</p>

                      {/* Skills */}
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {job.skills.map((skill) => (
                          <span key={skill} className="text-xs bg-surface-100 text-surface-600 px-2 py-0.5 rounded-full">{skill}</span>
                        ))}
                      </div>
                        ...
                        <div className="flex items-center gap-2">
                          {job.easyApply ? (
                            ...
                            <Link
                              ...
                              className="flex items-center gap-1.5 px-4 py-2 border border-surface-300 text-surface-700 rounded-lg text-sm font-medium hover:bg-surface-50 transition-colors"
                            >
          ...
          <div className="space-y-4">
            {/* Job Alerts Card */}
            <Card className="p-5 bg-gradient-to-br from-teal-50 to-info-50 border-teal-100">
            ...
            <Card className="p-5">
              ...
              <div className="space-y-2">
                {["إدارة المشاريع", "BIM", "السلامة المهنية", "الخرسانة المسلحة", "السباكة الحديثة", "أنظمة الذكاء"].map((skill) => (
                  <div key={skill} className="flex items-center justify-between p-2 rounded-lg hover:bg-surface-50 cursor-pointer">
                    <span className="text-sm text-surface-700">{skill}</span>
            ...
            <Card className="p-5">
              <h3 className="font-bold text-surface-900 mb-3 flex items-center gap-2">
                <Building2 className="w-4 h-4 text-flagship-500" />
                {language === "ar" ? "شركات نشطة" : language === "en" ? "Top Companies" : "فعال کمپنیاں"}
              </h3>
              <div className="space-y-3">
                {mockJobs.slice(0, 3).map((job) => (
                  <div key={job.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-50 cursor-pointer">
                    <div className={`w-10 h-10 rounded-lg ${job.company.color} flex items-center justify-center text-xs font-bold`}>
                      {job.company.initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-surface-900 truncate">{job.company.name}</p>
                      <p className="text-xs text-surface-500">{job.company.followers} {language === "ar" ? "متابع" : language === "en" ? "followers" : "فالوورز"}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Card className="p-5">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-surface-600">{language === "ar" ? "وظائف جديدة اليوم" : language === "en" ? "New Jobs Today" : "آج نئی ملازمتیں"}</span>
                  <span className="text-sm font-bold text-teal-600">12</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-surface-600">{language === "ar" ? "وظائف عاجلة" : language === "en" ? "Urgent Jobs" : "فوری ملازمتیں"}</span>
                  <span className="text-sm font-bold text-danger-600">8</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-surface-600">{language === "ar" ? "شركات توظف الآن" : language === "en" ? "Companies Hiring" : "کمپنیاں ملازمت دے رہی ہیں"}</span>
                  <span className="text-sm font-bold text-flagship-600">34</span>
                </div>
              </div>
            </Card>
          </div>
