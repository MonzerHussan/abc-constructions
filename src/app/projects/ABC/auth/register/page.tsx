  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInResult?.error) {
        setError("╪¬┘à ╪Ñ┘å╪┤╪º╪í ╪º┘ä╪¡╪│╪º╪¿. ╪º┘ä╪▒╪¼╪º╪í ╪¬╪│╪¼┘è┘ä ╪º┘ä╪»╪«┘ê┘ä");
        setLoading(false);
        return;
      }
      ...
      setError("╪¡╪»╪½ ╪«╪╖╪ú ╪ú╪½┘å╪º╪í ╪Ñ┘å╪┤╪º╪í ╪º┘ä╪¡╪│╪º╪¿");
      setLoading(false);
    }
  };

  const handleGoogleSignIn = () => {
    signIn("google", { callbackUrl: GOOGLE_ONBOARDING_CALLBACK });
  };
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="w-full pr-9 pl-3 py-2.5 border border-surface-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none" required />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} placeholder="05XXXXXXXX" className="w-full pr-9 pl-3 py-2.5 border border-surface-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none" required />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} placeholder="example@email.com" className="w-full pr-9 pl-3 py-2.5 border border-surface-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none" required />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? "text" : "password"} value={form.password} onChange={(e) => setForm({...form, password: e.target.value})} className="w-full pr-9 pl-9 py-2.5 border border-surface-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none" required minLength={8} />
                ...
                <div>
                  <label className="block text-sm font-medium text-surface-700 mb-1">{t("confirmPassword")}</label>
                  <div className="relative">
                    <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400" />
                    <input type={showPassword ? "text" : "password"} value={form.confirmPassword} onChange={(e) => setForm({...form, confirmPassword: e.target.value})} className="w-full pr-9 pl-3 py-2.5 border border-surface-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none" required minLength={8} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <span className="text-sm text-surface-600">
                  {t("agreeTerms")} <Link href="/terms" className="text-amber-600 hover:underline">{t("terms")}</Link> ┘ê <Link href="/privacy" className="text-amber-600 hover:underline">{t("privacy")}</Link>
                </span>
              </div>
          ...
          <p className="text-center mt-4 text-sm text-surface-600">
            {t("hasAccount")}{" "}
            <Link href="/projects/ABC/auth/login" className="text-amber-600 font-medium hover:text-amber-700">{t("login")}</Link>
          </p>
        </div>
      </div>
