...
export default function ProcurementDashboard() {
  ...
  const cards = [
    { label: t("totalPRs"), value: stats.totalPRs ?? "—", icon: FileText, href: "/projects/ABC/procurement/purchase-requests", color: "bg-info-50 text-info-600" },
    { label: t("pendingPRs"), value: stats.pendingPRs ?? "—", icon: AlertCircle, href: "/projects/ABC/procurement/purchase-requests?status=PENDING_APPROVAL", color: "bg-amber-50 text-amber-600" },
    { label: t("activeRFQs"), value: stats.totalRFQs ?? "—", icon: Send, href: "/projects/ABC/procurement/rfqs", color: "bg-success-50 text-success-600" },
    { label: t("totalPOs"), value: stats.totalPOs ?? "—", icon: ShoppingCart, href: "/projects/ABC/procurement/purchase-orders", color: "bg-flagship-50 text-flagship-600" },
    { label: t("pendingInvoices"), value: stats.pendingInvoices ?? "—", icon: Receipt, href: "/projects/ABC/procurement/invoices", color: "bg-danger-50 text-danger-600" },
  ]
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <Link href="/projects/ABC/procurement/purchase-requests/new" className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors text-sm">
            <Plus className="w-4 h-4" />
            {t("createPurchaseRequest")}
          </Link>
          <Link href="/projects/ABC/procurement/rfqs/new" className="flex items-center gap-2 px-4 py-2 bg-navy-800 text-white rounded-lg hover:bg-navy-900 transition-colors text-sm">
            <Plus className="w-4 h-4" />
            {t("createRfq")}
          </Link>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white border rounded-xl p-6">
          ...
          <div className="space-y-3">
            {[
              { href: "/projects/ABC/procurement/purchase-requests/new", label: language === "ar" ? "إنشاء طلب شراء جديد" : "Create Purchase Request", icon: FileText },
              { href: "/projects/ABC/procurement/rfqs/new", label: language === "ar" ? "إرسال طلب عرض سعر" : "Send RFQ", icon: Send },
              { href: "/projects/ABC/procurement/purchase-orders", label: language === "ar" ? "عرض أوامر الشراء" : "View Purchase Orders", icon: ShoppingCart },
              { href: "/projects/ABC/procurement/invoices", label: language === "ar" ? "إدارة الفواتير" : "Manage Invoices", icon: Receipt },
            ].map((item) => {
