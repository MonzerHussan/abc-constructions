...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      return Response.json(error(ErrorCodes.VALIDATION_ERROR, 'Invalid query parameters', Object.fromEntries(parsed.error.issues.map(i => [i.path.join('.'), i.message]))), { status: 400 });
    }
    ...
    return Response.json(error(ErrorCodes.INTERNAL_ERROR, 'حدث خطأ أثناء جلب طلبات الشراء'), { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return Response.json(error(ErrorCodes.INTERNAL_ERROR, 'حدث خطأ أثناء إنشاء طلب الشراء'), { status: 500 });
  }
}
