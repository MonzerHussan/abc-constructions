...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const lead = await prisma.lead.create({
      data: { firstName: body.firstName, lastName: body.lastName, email: body.email, phone: body.phone, company: body.company, jobTitle: body.jobTitle, source: body.source, status: body.status, score: body.score ?? 0, tags: body.tags ?? [], notes: body.notes, organizationId: body.organizationId, assignedToId: body.assignedToId, createdById: session.user.id },
      include: { assignedTo: { select: { id: true, name: true, email: true } }, createdBy: { select: { id: true, name: true, email: true } } },
    })
