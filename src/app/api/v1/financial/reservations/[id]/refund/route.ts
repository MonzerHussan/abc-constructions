...
import { refundFundsSchema } from '@/modules/financial/validators/financial-schemas';
...
export async function POST(
  ...
  try {
    ...
    const parsed = refundFundsSchema.parse(body);
    const result = await financialTrustService.refundFunds(id, parsed, session.user.id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.FINANCIAL_RESERVATION_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.FINANCIAL_RESERVATION_INVALID_TRANSITION, 'Cannot refund in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error refunding funds'), { status: 500 });
  }
}
