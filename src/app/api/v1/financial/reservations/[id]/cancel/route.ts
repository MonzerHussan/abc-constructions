...
import { cancelReservationSchema } from '@/modules/financial/validators/financial-schemas';
...
export async function POST(
  ...
  try {
    ...
    const result = await financialTrustService.cancelReservation(id, parsed, session.user.id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.FINANCIAL_RESERVATION_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.FINANCIAL_RESERVATION_INVALID_TRANSITION, 'Cannot cancel in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error cancelling reservation'), { status: 500 });
  }
}
