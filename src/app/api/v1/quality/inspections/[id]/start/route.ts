...
export const POST = withAuth(async (_request: NextRequest, { params }: { sessionUserId: string; params: Record<string, string> }) => {
  try {
    const { id } = params;
    const result = await qualityService.startInspection(id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.QUALITY_INSPECTION_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.QUALITY_INSPECTION_INVALID_TRANSITION, 'Cannot start inspection in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error starting inspection'), { status: 500 });
  }
});