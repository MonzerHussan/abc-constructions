...
export async function POST(
  ...
  try {
    ...
    const result = await qualityService.passInspection(id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.QUALITY_INSPECTION_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.QUALITY_INSPECTION_INVALID_TRANSITION, 'Cannot pass inspection in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error passing inspection'), { status: 500 });
  }
}
