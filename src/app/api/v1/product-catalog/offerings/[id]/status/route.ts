...
const actionSchema = z.object({
  action: z.enum(['discontinue', 'restock', 'activate', 'markComingSoon']),
});
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await productCatalogService.transitionOffering(id, action);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ProductCatalogErrors.OFFERING_INVALID_TRANSITION) {
        return NextResponse.json(error(ProductCatalogErrors.OFFERING_INVALID_TRANSITION, 'Cannot perform this action in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error('INTERNAL_ERROR', 'Error updating offering status'), { status: 500 });
  }
}
