...
import { uploadDataSheetSchema } from '@/modules/product-catalog/validators/product-catalog-schemas';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = uploadDataSheetSchema.parse(body);
    const sheet = await productCatalogService.uploadDataSheet(id, parsed);
    ...
    return NextResponse.json(error('INTERNAL_ERROR', 'Error uploading data sheet'), { status: 500 });
  }
}
