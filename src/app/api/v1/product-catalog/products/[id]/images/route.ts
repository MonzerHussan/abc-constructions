...
import { uploadImageSchema } from '@/modules/product-catalog/validators/product-catalog-schemas';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const image = await productCatalogService.uploadImage(id, parsed);
    return NextResponse.json(success(image, createRequestId()), { status: 201 });
  } catch (err: unknown) {
    ...
    return NextResponse.json(error('INTERNAL_ERROR', 'Error uploading image'), { status: 500 });
  }
}
