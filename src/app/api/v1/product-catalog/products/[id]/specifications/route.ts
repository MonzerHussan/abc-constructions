...
import { addSpecificationSchema } from '@/modules/product-catalog/validators/product-catalog-schemas';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = addSpecificationSchema.parse(body);
    const spec = await productCatalogService.addSpecification(id, parsed);
    return NextResponse.json(success(spec, createRequestId()), { status: 201 });
  } catch (err: unknown) {
    ...
    return NextResponse.json(error('INTERNAL_ERROR', 'Error adding specification'), { status: 500 });
  }
}
