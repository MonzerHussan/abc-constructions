...
export async function POST(
  ...
  try {
    ...
    const quotation = await quotationService.submit(id, session.user.id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.PROCUREMENT_QUOTATION_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.PROCUREMENT_QUOTATION_INVALID_TRANSITION, 'Cannot submit quotation in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error submitting quotation'), { status: 500 });
  }
}
