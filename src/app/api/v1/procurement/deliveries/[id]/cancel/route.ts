  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION, 'Cannot cancel delivery in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error cancelling delivery'), { status: 500 });
  }
});