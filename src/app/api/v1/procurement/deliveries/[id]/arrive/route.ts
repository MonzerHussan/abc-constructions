...
export async function POST(
  ...
  try {
    ...
    const delivery = await deliveryService.arrive(id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION, 'Cannot mark delivery arrived in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error marking delivery arrived'), { status: 500 });
  }
}
