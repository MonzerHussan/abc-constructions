...
export const POST = withAuth(async (_request: NextRequest, { params }: { sessionUserId: string; params: Record<string, string> }) => {
  try {
    const { id } = params;
    const delivery = await deliveryService.receive(id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.PROCUREMENT_DELIVERY_INVALID_TRANSITION, 'Cannot receive delivery in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error receiving delivery'), { status: 500 });
  }
});