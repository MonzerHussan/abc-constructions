...
export async function POST(
  ...
  try {
    ...
    const po = await purchaseOrderService.issue(id, session.user.id);
    ...
    if (err instanceof Error) {
      ...
      if (err.message === ErrorCodes.PROCUREMENT_PO_INVALID_TRANSITION) {
        return NextResponse.json(error(ErrorCodes.PROCUREMENT_PO_INVALID_TRANSITION, 'Cannot issue PO in current status'), { status: 400 });
      }
    }
    return NextResponse.json(error(ErrorCodes.INTERNAL_ERROR, 'Error issuing purchase order'), { status: 500 });
  }
}
