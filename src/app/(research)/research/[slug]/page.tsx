...
export default function CampaignDetailPage() {
  ...
  if (!campaign) return <div className="min-h-screen flex items-center justify-center text-surface-500"><div className="text-center"><ClipboardList className="mx-auto h-16 w-16 mb-4 opacity-50" /><p className="text-xl">?????? ??? ??????</p><Link href="/research" className="text-info-600 mt-4 inline-block">?????? ???????</Link></div></div>
    ...
    <div className="min-h-screen bg-surface-50" dir="rtl">
      <div className="bg-gradient-to-br from-info-600 to-flagship-700 text-white">
        <div className="max-w-5xl mx-auto px-6 py-12">
          <Link href="/research" className="flex items-center gap-2 text-info-200 hover:text-white mb-6"><ArrowLeft size={20} /> ?????? ???????</Link>
          ...
          {campaign.surveys && campaign.surveys.length > 0 ? (
            <div className="space-y-4">
              {campaign.surveys.map(s => (
                <Link key={s.id} href={"/research/" + campaign.slug + "/survey/" + s.id} className="block bg-white border rounded-xl p-5 hover:shadow-md transition">
