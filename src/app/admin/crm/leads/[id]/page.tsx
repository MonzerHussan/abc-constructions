...
export default function LeadEditPage() {
  ...
  useEffect(() => {
    ...
    if (params?.id) {
      fetch(`/api/crm/leads/${params.id}`)
        ...
        .catch(() => { setError("لم يتم العثور على العميل المحتمل"); router.push("/admin/crm/leads") })
    }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        <Link href="/admin/crm/leads" className="flex items-center gap-2 text-surface-600 hover:text-surface-900 mb-4"><ArrowRight size={18} /> العودة</Link>
        <h1 className="text-2xl font-bold">تعديل عميل محتمل</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border rounded-xl p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/admin/crm/leads" className="px-6 py-2.5 border border-surface-300 rounded-xl text-surface-700 hover:bg-surface-50">إلغاء</Link>
        </div>
      </form>
    </div>
  )
