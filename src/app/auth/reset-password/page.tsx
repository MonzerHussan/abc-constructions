...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.message ?? "حدث خطأ أثناء إعادة التعيين")
      } else {
        setDone(true)
      }
    } catch {
    ...
    return (
      <div className="bg-success-50 border border-success-200 text-success-700 text-sm px-4 py-3 rounded-xl space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">تم تحديث كلمة المرور بنجاح</span>
        </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {email && (
        <div className="bg-surface-100 text-surface-700 text-sm px-4 py-2.5 rounded-xl">
          إعادة تعيين كلمة المرور لـ <b>{email}</b>
        </div>
      )}
      ...
      <div>
        <label className="block text-sm font-medium text-surface-700 mb-1.5">تأكيد كلمة المرور</label>
        <div className="relative">
          <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-400" />
          <input
            type={showPassword ? "text" : "password"}
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <div className="mb-8">
            ...
            <h1 className="text-2xl font-bold text-surface-900">إعادة تعيين كلمة المرور</h1>
            <p className="text-surface-600 mt-1">أدخل كلمة مرور جديدة لحسابك</p>
          </div>
      ...
      <div className="hidden lg:flex lg:flex-1 gradient-hero items-center justify-center p-12">
        <div className="text-center text-white flex max-col gap-2 items-center">
          ...
          <div className="text-right">
            <h2 className="text-3xl font-bold mb-2">استعادة وصولك</h2>
            <p className="text-white/80">كلمة مرور جديدة، حماية أقوى</p>
          </div>
        </div>
      </div>
    </div>
